...
export class Paddle implements IReflectiveObject{
    ...
    Distance(trajectory: Trajectory): number {
        let distanceX = this.position.x - trajectory.position.x
        let distance:number = distanceX/trajectory.vector.x
        if(distance > trajectory.distance
            || distance <= 0){
            return 0;  //path does not intersect
        }
        let distanceY = distanceX / trajectory.vector.x * trajectory.vector.y
        let intersectionHeigt = trajectory.position.y + distanceY;
        //0 is top of the paddle, 0.5 is middle, 1 is end of paddle
        let intersectFractionPaddle = (intersectionHeigt - this.position.y)/this.size.y
    ...
    Reflect(trajectory: Trajectory): Trajectory {
        ...
        let intersectionPoint:Vector2 = new Vector2(this.position.x,intersectionHeigt);
        let distanceLeft = trajectory.distance - this.Distance(trajectory)
        let newDirection:number = this.newDirection(intersectionHeigt);
        if(trajectory.vector.x > 0){
            newDirection = Math.PI - newDirection;
            console.log("adjusted")
        }
        let vector:Vector2 = new Vector2(Math.cos(newDirection),Math.sin(newDirection))
        console.log(vector)
